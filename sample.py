...
def sample(model, seqs=[[]], n_samples=1, beam_search=None, ignore_unk=False, normalize=False, alpha=1, verbose=False): 
    if beam_search:
        ...
        for idx, seq in enumerate(seqs):
            ...
            gen_queries, gen_costs, gen_ranks = beam_search.search(seq, n_samples, ignore_unk=ignore_unk) 
...
def context_to_indices(contexts, model):
    ...
    for ctx_indx, ctx_text in enumerate(contexts):
        ...
        if 0 in indx:
            ...
            seqs[ctx_indx] = list(itertools.chain(*[model.words_to_indices(c.split(), add_se=True) for c in ctx_queries]))
    return seqs
